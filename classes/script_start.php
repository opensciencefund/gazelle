...
if (!isset($argv) && !empty($_SERVER['HTTP_HOST'])) { //Skip this block if running from cli or if the browser is old and shitty
	if ($_SERVER['SERVER_PORT'] == 80 && $_SERVER['HTTP_HOST'] == 'www.'.NONSSL_SITE_URL) { header('Location: http://'.NONSSL_SITE_URL.$_SERVER['REQUEST_URI']); die(); }
	if ($_SERVER['SERVER_PORT'] == 443 && $_SERVER['HTTP_HOST'] == 'www.'.NONSSL_SITE_URL) { header('Location: https://'.SSL_SITE_URL.$_SERVER['REQUEST_URI']); die(); }
	if(SSL_SITE_URL != NONSSL_SITE_URL) {
		if ($_SERVER['SERVER_PORT'] == 80 && $_SERVER['HTTP_HOST'] == SSL_SITE_URL) { header('Location: https://'.SSL_SITE_URL.$_SERVER['REQUEST_URI']); die(); }
		if ($_SERVER['SERVER_PORT'] == 443 && $_SERVER['HTTP_HOST'] == NONSSL_SITE_URL) { header('Location: http://'.NONSSL_SITE_URL.$_SERVER['REQUEST_URI']); die(); }
	}
}
...
function user_info($UserID) {
	...
	if(!is_array($UserInfo)) {
		...
		if($DB->record_count() == 0) { // Deleted user, maybe?
			$UserInfo = array('ID'=>'','Username'=>'','PermissionID'=>0,'Artist'=>false,'Donor'=>false,'Warned'=>'0000-00-00 00:00:00','Avatar'=>'','Enabled'=>0,'Title'=>'', 'CatchupTime'=>0);
		} else {
...
function user_heavy_info($UserID) {
	...
	if(!$HeavyInfo || $HeavyInfo['StyleID'] == 1) {
		$DB->query("SELECT
			m.Invites,
			m.torrent_pass,
			m.IP,
...
function ip2unsigned($IP) {
	$Long = ip2long($IP);
	if($Long<0) { // php's ip2long is signed, MySQL's INET_ATON is unsigned. Note - this may break on 32bit servers.
		$Long = (1024*1024*1024*4)+$Long;
	}
	return $Long;
}
...
function old_geoip($IP) {
	...
	if(empty($Countries[$IP])) {
		...
		if(!$Country) {
			...
			$Return = array_pop(explode('.',$Return));
...
//	$DB->query("DELETE FROM sphinx_delta WHERE ID=$GroupID");
...
//	$DB->query("DELETE FROM sphinx_requests_delta WHERE ID = ".$RequestID);
	...
	$DB->query("INSERT INTO sphinx_requests_delta
					(ID, ArtistList)
				SELECT RequestID,
					GROUP_CONCAT(aa.Name separator ' ')
...
function selected($Name, $Value, $Attribute='selected') {
