	...
	function parse($Str) {
		...
		while($i<$Len) {
			...
			if($TagName == 'img' && !empty($Tag[3][0])) { //[img=...]
				...
				$Block = substr($Str, $i, $CloseTag); // Get the URL
		...
		foreach($Array as $Block) {
			...
			switch($Block['Type']) {
				...
				case 'wiki':
					...
					break;
				case 'tex':
					$Str.='<img src="'.STATIC_SERVER.'blank.gif" onload="if (this.src.substr(this.src.length-9,this.src.length) == \'blank.gif\') { this.src = \'http://chart.apis.google.com/chart?cht=tx&chf=bg,s,FFFFFF00&chl='.urlencode($Block['Val']).'&chco=\' + hexify(getComputedStyle(this.parentNode,null).color); }" />';
					break;
				case 'pre':
				...
				case 'inlineurl':
					if(!$this->valid_url($Block['Attr'])) {
						...
						$LocalURL = $this->local_url($Block['Attr']);
						if($LocalURL) {
							$Str.='<a href="'.$LocalURL.'">'.$LocalURL.'</a>';
						} else {
