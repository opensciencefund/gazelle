...
$DB->query("SELECT ID FROM ip_bans WHERE ".ip2long($_SERVER['REMOTE_ADDR'])." BETWEEN FromIP AND ToIP LIMIT 1");
list($BanID)=$DB->next_record();

if ($BanID && false) {
	// Will display a message about their IP being banned
	...
	function log_attempt($UserID) {
		...
		if($AttemptID) { // User has attempted to log in recently
			...
			if ($Attempts>5) { // Only 6 allowed login attempts, ban user's IP
				...
				if (!empty($UserID)) {
					$DB->query("UPDATE users_info SET AdminComment = CONCAT('".sqltime()." - 6 Failed login attempts from ".$_SERVER['REMOTE_ADDR']."\n\n', AdminComment) WHERE UserID = ".$UserID);
				}
	...
	if(isset($_POST['username']) && !empty($_POST['username']) && isset($_POST['password']) && !empty($_POST['password'])) {
		...
		if(!$Err) {
			...
			$DB->query("SELECT
				...
				AND Username<>''");
			list($UserID,$PermissionID,$PassHash,$Secret,$Enabled)=$DB->next_record();
			if (strtotime($BannedUntil)<time()) {
				if ($UserID && $PassHash==make_hash($_POST['password'],$Secret)) {
					if ($Enabled == 1) {
						...
						if (isset($Permissions['Permissions']['site_disable_ip_history'])) { $_SERVER['REMOTE_ADDR'] = '127.0.0.1'; }
