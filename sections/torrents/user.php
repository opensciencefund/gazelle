...
$SearchWhere = array();
if(!empty($_GET['format']) && in_array($_GET['format'], $Formats)) {
...
if(isset($_GET['log']) && in_array($_GET['log'], array('1','0', '100'))) {
	if($_GET['log'] == '100') {
...
if((empty($_GET['search']) || trim($_GET['search']) == '') && $Order!='Name') {
	...
	$DB->query("INSERT INTO t (GroupID, TorrentID, Time, CategoryID, Seeders, Leechers, Snatched, Name, Size) SELECT
		t.GroupID, 
		t.ID As TorrentID, 
		$Time AS Time, 
		tg.CategoryID,
	...
	$Words = explode(' ', db_string($_GET['search']));
