...
?>
		<a href="#torrents_<?=str_replace(" ", "_", strtolower($ReleaseTypes[$ReleaseID]))?>">[<?=$ReleaseTypes[$ReleaseID].($ReleaseTypes[$ReleaseID] == "Remix" ? "es" : "s")?>]</a>
<?
	}
?>
	</div>
...
foreach ($TorrentList as $GroupID=>$Group) {
	...
	if($ReleaseType!=$LastReleaseType) {
		$ReleaseTypeName = $ReleaseTypes[$ReleaseType].'s';
		if($ReleaseTypeName == 'Remixs') { $ReleaseTypeName = 'Remixes'; }
...
<?		} ?>
			<table class="torrent_table" id="torrents_<?=$ReleaseTypeLabel?>">
				<tr class="colhead_dark">
					<td width="70%"><a href="#">&uarr;</a>&nbsp;<strong><?=$ReleaseTypeName?></strong> (<a href="#" onclick="$('.releases_<?=$ReleaseType?>').toggle();return false;">View</a>)</td>
					<td>Size</td>
...
<? } if (check_perms('site_torrents_notify')) { ?>
		<a href="artist.php?action=notify&amp;artistname=<?=urlencode($Name)?>">[Notify of new uploads]</a>
